...
privateControllers.controller('RelyingPartyTemplateController', ['$scope', '$location', 'RelyingPartyAdmin', 'Upload', 'global',
  function($scope, $location, RelyingPartyAdmin, Upload, global) {
    ...
    $scope.getRelyingParty = function(rpId) {
        (new RelyingPartyAdmin(rpId, '')).get(
            function(data) {
                ...
                $scope.RPSuccessMessage = 'Relying Party retrieved successfully';
            }, function(error) {
                $scope.RPFailureMessage = 'There was a problem retrieving the Relying Party';
            }
        );
    }
    ...
    $scope.modifyRP = function(relyingParty) {
      if (relyingParty.uuid) {
        $scope.updateRelyingParty(relyingParty)
      } else {
        $scope.createRp(relyingParty)
      }
    }

    $scope.updateRelyingParty = function(relyingParty) {
        console.log(relyingParty)
        ...
        (new RelyingPartyAdmin(null)).update(relyingParty,
            function(data) {
                $scope.relyingParty = data;
                $scope.RPSuccessMessage = 'The Relying Party was updated successfully';
            }, function(error) {
                if( error.status == 409 ) {
                    $scope.RPFailureMessage = 'There was a problem with the update.  A relying party with that name already exists.';
                } else {
                    $scope.RPFailureMessage = 'There was a problem updating the Relying Party';
                }
            }
        );
    }
    ...
    $scope.clearMessages = function() {
        $scope.RPSuccessMessage = null;
        $scope.RPFailureMessage = null;
    }
    ...
    $scope.createRp = function() {
      ...
      (new RelyingPartyAdmin(null)).create($scope.relyingParty,
        function(data) {
          $scope.relyingParty = data;
          $scope.RPSuccessMessage = 'Relying Party created successfully';
        }, function(error) {
          console.log(error)
          console.log(error.status)
          if( error.status === '409' ) {
            $scope.RPFailureMessage = 'There was a problem creating the Relying Party. A relying party with that name already exists.';
          } else if( error.status === '417' ) {
            $scope.RPFailureMessage = 'There was a problem creating the Relying Party. Image sizes are too large.';
          } else {
            $scope.RPFailureMessage = 'There was a problem creating the Relying Party';
          }
        }
      );
    };
